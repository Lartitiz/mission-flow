...
const SITE_NAME = "nowadays-mission-flow"
// SENDER_DOMAIN is the verified sender subdomain FQDN (e.g., "notify.example.com").
// It MUST match the subdomain delegated to Lovable's nameservers — never the root domain.
// The email API looks up this exact domain; a mismatch causes "No email domain record found".
const SENDER_DOMAIN = "notify.nowadaysagency.com"
// FROM_DOMAIN is the domain shown in the From: header (e.g., "example.com").
// When display_from_root is enabled, this can be the root domain for cleaner branding,
// even though actual sending uses the subdomain above.
const FROM_DOMAIN = "nowadaysagency.com"
...
// Auth note: verify_jwt = true validates the JWT at the gateway, but the anon
// key (shipped in the frontend bundle) passes that check. Sending from the
// verified domain must stay reserved to service-role callers (other edge
// functions), so we check the role claim explicitly.
